...
class RiggingToolsCollection:
  def __init__(self, scripts_directory):
    ...
    self.config_file = os.path.join(
        os.path.dirname(scripts_directory), "rigging_tools_config.json"
    )
    self.button_order = []
    self.categories = []
    ...
    # Updated default to include a 4th value for Alpha (Opacity)
  ...
  def scan_scripts_directory(self):
    """Scan the directory for Python scripts and store their paths."""
    if not os.path.exists(self.scripts_directory):
      cmds.warning(f"Directory does not exist: {self.scripts_directory}")
      return

    if self.scripts_directory not in sys.path:
      sys.path.append(self.scripts_directory)

    for file_name in os.listdir(self.scripts_directory):
      if file_name.endswith(".py") and not file_name.startswith("__"):
        module_name = os.path.splitext(file_name)[0]
        self.tool_scripts[module_name] = os.path.join(
            self.scripts_directory, file_name
        )

  def load_config(self):
    """Load button order, color settings, categories, and tool help."""
    (
        self.button_order,
  ...
  def save_config(self):
    """Save all configurations including help data to file."""
    try:
  ...
  def run_script(self, module_name, *args):
    """Run a selected script through the runner module."""
    run_tool(self.tool_scripts, module_name)
  ...
  def delete_tool_help(self, module_name, *args):
    if module_name in self.tool_help:
      ...
      if result == "Delete":
        ...
        self._defer_grid_rebuild()
        cmds.inViewMessage(
            amg=f"Deleted help for {tool_display}", pos="midCenter", fade=True
        )
    else:
      cmds.inViewMessage(
          amg="No help exists for this tool.", pos="midCenter", fade=True
      )

  def _refresh_help_selector(self):
    if not cmds.optionMenu("helpToolSelector", exists=True):
      return

    menu_items = (
        cmds.optionMenu("helpToolSelector", query=True, itemListLong=True) or []
    )
    ...
    if self.button_order:
      for tool in self.button_order:
        has_help = bool(
            self.tool_help.get(tool, {}).get("description")
            or self.tool_help.get(tool, {}).get("links")
        )
      ...
      cmds.menuItem(
          label="No tools available", enable=False, parent="helpToolSelector"
      )
  ...
  def _get_selected_help_module(self):
    ...
    for tool in self.button_order:
      if tool.replace("_", " ").title() == cleaned:
        return tool
    return None

  def _update_selected_help_status(self, *args):
    if not cmds.text("helpStatusText", exists=True):
      return
  ...
  def _open_selected_tool_help(self, *args):
    module_name = self._get_selected_help_module()
    if module_name:
      self.show_tool_help_dialog(module_name)

  def _edit_selected_tool_help(self, *args):
    module_name = self._get_selected_help_module()
    if module_name:
      self.edit_tool_help_dialog(module_name)

  def _delete_selected_tool_help(self, *args):
    module_name = self._get_selected_help_module()
    if module_name:
      self.delete_tool_help(module_name)

  def create_help_settings(self, parent_layout):
    self.help_frame = cmds.frameLayout(
        label="Tool Help & Video Settings",
        collapsable=True,
        collapse=True,
        parent=parent_layout,
        marginWidth=5,
        marginHeight=5,
        collapseCommand=self.adjust_window_height,
        expandCommand=self.adjust_window_height,
    )

    help_layout = cmds.columnLayout(
        adjustableColumn=True, columnAttach=("both", 5), rowSpacing=4
    )
    cmds.text(
        label="(Tip: You can also right-click any tool button to view or edit help)",
        align="left",
    ...
    cmds.optionMenu(
        "helpToolSelector", changeCommand=self._update_selected_help_status
    )
    cmds.setParent(help_layout)

    cmds.text(
        "helpStatusText",
        label="Status: Select a tool",
        align="left",
    ...
    btn_row = cmds.rowLayout(
        numberOfColumns=3,
        adjustableColumn=1,
        columnWidth3=(120, 130, 90),
        parent=help_layout,
    )
    cmds.button(
        label="View Help",
        height=28,
        command=self._open_selected_tool_help,
        annotation="View help and open video links",
    )
    cmds.button(
        label="Add / Edit Help...",
        height=28,
        command=self._edit_selected_tool_help,
        annotation="Add or edit description and video links",
    )
    cmds.button(
        label="Delete Help",
        height=28,
        command=self._delete_selected_tool_help,
        annotation="Delete help settings for selected tool",
    )
  ...
  def swap_buttons(self, source_index, target_index):
    ...
    if 0 <= source_index < len(self.button_order) and 0 <= target_index < len(
        self.button_order
    ):
      self.button_order[source_index], self.button_order[target_index] = (
          self.button_order[target_index],
          self.button_order[source_index],
      )
      self.save_config()
      self._defer_grid_rebuild()
      cmds.inViewMessage(
          amg="Button positions updated", pos="midCenter", fade=True
      )
  ...
  def start_drag(self, button_index, *args):
    ...
    cmds.inViewMessage(
        amg=f"Marked <hl>{source_name}</hl>. Right-click a target and choose Drop Here.",
        pos="midCenter",
        fade=True,
    )
  ...
  def drop_on_target(self, target_index, *args):
    ...
    if source_index is None:
      cmds.inViewMessage(
          amg="Choose Start Drag on a tool first.", pos="midCenter", fade=True
      )
      return
  ...
  def drop_tool_button(self, target_module_name, messages, *args):
    if len(messages) < 2 or messages[0] != "RiggingToolsCollectionTool":
      return
  ...
  def _refresh_group_selector(self):
    if not cmds.optionMenu("groupManagerSelector", exists=True):
      return
    ...
    if self.categories:
      ...
      cmds.menuItem(
          label="No custom groups", enable=False, parent="groupManagerSelector"
      )
  ...
  def get_blended_bgc(self, color_values):
    """Simulate Maya transparency by blending the RGBA color with the UI background."""
    r = color_values[0]
    g = color_values[1]
    b = color_values[2]
    a = color_values[3] if len(color_values) > 3 else 1.0

    bg = 0.25  # Match the layout background color precisely
    return [
  ...
  def select_button_for_editing(self, module_name, *args):
    ...
    # Extract Alpha safely (handles old saved 3-color setups)
  ...
  def create_color_settings(self, parent_layout):
    ...
    preview_row = cmds.rowLayout(
        numberOfColumns=2, adjustableColumn=1, columnAlign2=("left", "center")
    )
    ...
    cmds.floatSliderGrp(
        "redSlider",
        label="Red", field=True, minValue=0.0, maxValue=1.0, fieldMinValue=0.0, fieldMaxValue=1.0,
        value=self.current_button_color[0], dragCommand=self.update_button_color, changeCommand=self.update_button_color,
    )
    cmds.floatSliderGrp(
        "greenSlider",
        label="Green", field=True, minValue=0.0, maxValue=1.0, fieldMinValue=0.0, fieldMaxValue=1.0,
        value=self.current_button_color[1], dragCommand=self.update_button_color, changeCommand=self.update_button_color,
    )
    cmds.floatSliderGrp(
        "blueSlider",
        label="Blue", field=True, minValue=0.0, maxValue=1.0, fieldMinValue=0.0, fieldMaxValue=1.0,
        value=self.current_button_color[2], dragCommand=self.update_button_color, changeCommand=self.update_button_color,
    )
    cmds.floatSliderGrp(
        "alphaSlider",
        label="Opacity", field=True, minValue=0.0, maxValue=1.0, fieldMinValue=0.0, fieldMaxValue=1.0,
    ...
    for row_start in range(0, len(presets), 5):
      ...
      for column_index, (name, values) in enumerate(row_presets):
        preset_button = cmds.button(
            ...
            command=lambda x, c=values: self.preset_color(c),
            height=26,
            recomputeSize=False,
            parent=preset_row,
        )
        ...
        if column_index == 0:
          attachments.append((preset_button, "left", 0))
        else:
          cmds.formLayout(preset_row, edit=True, attachPosition=(preset_button, "left", 3, left_pos))

        if column_index == len(row_presets) - 1:
          attachments.append((preset_button, "right", 0))
        else:
          cmds.formLayout(preset_row, edit=True, attachPosition=(preset_button, "right", 3, right_pos))
  ...
  def _create_responsive_row(self, parent_layout, row_modules, row_start_index):
    ...
    for column_index, module_name in enumerate(row_modules):
      ...
      # Use the blended color to simulate opacity safely
      blended_color = self.get_blended_bgc(button_color)

      # Help status & annotation
      ...
      button = cmds.button(
          button_name,
          label=display_name,
          command=lambda x, mn=module_name: self.run_script(mn),
          annotation=button_annotation,
          backgroundColor=blended_color,
      ...
      if column_index == 0:
        attachments.append((button, "left", 0))
      else:
        cmds.formLayout(row, edit=True, attachPosition=(button, "left", self.grid_spacing, left_position))
        
      if column_index == column_count - 1:
        attachments.append((button, "right", 0))
      else:
        cmds.formLayout(row, edit=True, attachPosition=(button, "right", self.grid_spacing, right_position))
      ...
      if has_help:
        cmds.menuItem(label="Delete Help", command=lambda x, mn=module_name: self.delete_tool_help(mn))
  ...
  def create_grid_layout(self, parent_layout):
    ...
    if not module_names:
      cmds.text(label="No Python scripts found in the directory.", align="center", parent=parent_layout)
      return
    ...
    uncategorized_tools = [
        ...
        if self.button_categories.get(module) not in self.categories
    ]

    if uncategorized_tools or not group_names:
      group_names.append(self.uncategorized_label)

    for category in group_names:
      if category == self.uncategorized_label:
        group_tools = uncategorized_tools
      else:
        group_tools = [
            module for module in module_names
            if self.button_categories.get(module) == category
        ]

      group_frame = cmds.frameLayout(
          label=category, collapsable=True, collapse=False, marginWidth=6, marginHeight=6, parent=grid_layout
      )
      ...
      if group_tools:
        ...
        cmds.text(label="No tools assigned to this group yet.", align="left", parent=group_layout)
  ...
  def _install_resize_filter(self):
    try:
      ...
      class ResizeFilter(QtCore.QObject):
        def eventFilter(self, watched, event):
          if event.type() == QtCore.QEvent.Resize:
            owner._schedule_responsive_grid()
          return False
      ...
      cmds.warning(f"Responsive resize support could not be installed: {error}")
  ...
  def refresh_content(self, *args):
    ...
    if cmds.layout(self.content_layout, exists=True):
      ...
      self._rebuild_grid()
      cmds.inViewMessage(amg="Tools refreshed", pos="midCenter", fade=True)
    else:
      self.show_ui()
  ...
  def show_ui(self):
    ...
    window = cmds.window(
        self.window_name,
        title="Rigging Tools Collection (SAT)",
    ...
    cmds.text(label="Rigging Tools Collection", font="boldLabelFont", height=30)
    cmds.text(
    ...
    # -----------------------------------------------------------
    # Redesigned "Proper" Footer Layout
    # -----------------------------------------------------------
    footer_layout = cmds.formLayout(
        parent=main_form, 
        height=44, 
    ...
    refresh_button = cmds.button(
        ...
        height=30,
        width=140,
        backgroundColor=[0.2, 0.5, 0.35], # Distinct Green Action Button
    ...
    close_button = cmds.button(
        ...
        height=30,
        width=120,
        backgroundColor=[0.6, 0.25, 0.25], # Distinct Red Action Button
    ...
    cmds.formLayout(
        ...
        edit=True,
        attachForm=[
            (refresh_button, "top", 7),
            (refresh_button, "bottom", 7),
            (close_button, "top", 7),
            (close_button, "bottom", 7),
            (close_button, "right", 10),
        ],
        attachControl=[
    ...
    cmds.formLayout(
        ...
        attachForm=[
            (header_column, "top", 0),
            (header_column, "left", 0),
            (header_column, "right", 0),
            (footer_layout, "left", 0),
            (footer_layout, "right", 0),
            (footer_layout, "bottom", 0),
            (scroll_layout, "left", 0),
            (scroll_layout, "right", 0),
        ],
        attachControl=[
            (scroll_layout, "top", 5, header_column),
            (scroll_layout, "bottom", 5, footer_layout),
        ],
    )
...
def launch_rigging_tools_collection():
  scripts_directory = r"C:/Users/sat/Documents/maya/scripts/sattools/tools"
  tools_collection = RiggingToolsCollection(scripts_directory)
