...
class MayaPipInstallerUI(QtWidgets.QDialog):
    ...
    def _build_ui(self):
        ...
        self.install_btn.setStyleSheet("background-color: #3a7a3a; color: white; font-weight: bold; border-radius: 4px;")
        self.install_btn.clicked.connect(self._start_install)

        self.uninstall_btn = QtWidgets.QPushButton("Uninstall")
        self.uninstall_btn.setFixedHeight(34)
        self.uninstall_btn.setStyleSheet("background-color: #7a3a3a; color: white; font-weight: bold; border-radius: 4px;")
        ...
        self.log_output.setStyleSheet("background: #1e1e1e; color: #c8c8c8; font-family: Consolas, monospace; font-size: 11px;")
    ...
    def _get_maya_user_site_packages(self):
        if os.name == "nt":
            path = os.path.join(
                ...
                self.maya_version, "scripts", "site-packages"
            )
        else:
    ...
    def _scan_installed_packages(self, target_path, maya_system_site, maya_site, user_site):
        ...
        def add_package(name, version, path_label):
            ...
            record = found.setdefault(key, {"name": name, "version": version or "", "locations": []})
        ...
        def scan_folder(folder, path_label):
            ...
            for entry in entries:
                lower = entry.lower()
                if entry in skip_exact or any(lower.startswith(prefix) for prefix in skip_prefixes): continue
                if lower.endswith((".dist-info", ".egg-info", ".pth", ".pyd", ".so", ".dll")): continue
        ...
        for record in sorted(found.values(), key=lambda item: item["name"].lower()):
            label = record["name"]
            if record["version"]: label += "==" + record["version"]
            label += "   " + " ".join(record["locations"])
            items.append(label)
        return items

    def _refresh_installed_async(self):
        if self._installed_scan_active: return
    ...
    def _show_installed_packages(self, items):
        ...
        self.installed_list.clear()
        if items: self.installed_list.addItems(items)
        else: self.installed_list.addItem("No packages detected.")
    ...
    def _append_to_maya_env(self, path_to_add):
        try:
            maya_env_dir = os.path.join(os.path.expanduser("~"), "Documents", "maya", self.maya_version)
    ...
    def _package_exists_in_path(self, package_name, folder):
        ...
        try: entries_lower = {e.lower(): e for e in os.listdir(folder)}
        except OSError: return False

        for variant in (pkg_under.lower(), pkg_dash.lower()):
            if variant in entries_lower: return True
            if variant + ".py" in entries_lower: return True
            prefix_di = variant + "-"
    ...
    def _build_cmd(self, action, packages=None):
        ...
        if not packages: return None, None, "No package name entered."
            ...
            if use_user_flag:
                ...
                if target:
                    ...
                    if is_protected and not self.chk_admin.isChecked():
                        return None, None, ("Install path is inside a protected system directory:\n  {}\n\nEnable 'Use Administrator' to install there, or choose --user.").format(target)
                    if not is_protected: os.makedirs(target, exist_ok=True)
                    cmd += ["--target", target]
                    install_path = target
            if self.chk_upgrade.isChecked(): cmd.append("--upgrade")
            if self.chk_no_deps.isChecked(): cmd.append("--no-deps")
        elif action == "uninstall": cmd.append("-y")
    ...
    def _run_elevated_process(self, cmd):
        ...
        try:
            arguments = ",\n    ".join(self._powershell_quote(argument) for argument in cmd[1:])
            elevated_script = (
                ...
                "exit $exitCode\n"
            ).format(mayapy=self._powershell_quote(cmd[0]), arguments=arguments, output=self._powershell_quote(output_path))
            with open(script_path, "w", encoding="utf-8") as script_file: script_file.write(elevated_script)

            ps_arguments = "-NoProfile -ExecutionPolicy Bypass -File {}".format(subprocess.list2cmdline([script_path]))
            launcher = (
            ...
            result = subprocess.run(["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", launcher], capture_output=True, text=True)

            try:
                with open(output_path, "r", encoding="utf-8", errors="replace") as output_file:
                    elevated_output = output_file.read().strip()
                if elevated_output: result.stdout = elevated_output
            except OSError: pass
            return result
        finally:
    ...
    def _remove_package_from_folder(self, package_name, folder):
        ...
        patterns = [
            os.path.join(folder, pkg_clean), os.path.join(folder, pkg_clean + ".py"), os.path.join(folder, pkg_clean + "*.pyd"),
            os.path.join(folder, pkg_clean + "*.dll"), os.path.join(folder, pkg_clean + "-*.dist-info"), os.path.join(folder, pkg_clean + "-*.egg-info"),
            os.path.join(folder, pkg_dash + "-*.dist-info"), os.path.join(folder, pkg_dash + "-*.egg-info"),
        ]
        ...
        for pattern in patterns:
            for path in glob.glob(pattern):
                try:
                    ...
                    removed_any = True
                except Exception as e: self.log_signal.emit("⚠ Failed to remove {} ({})".format(path, e))

        if not removed_any: self.log_signal.emit("⚠ No matching files found in: {}".format(folder))
    ...
    def _run_pip(self, action):
        ...
        if action == "install":
            ...
            if not self.chk_upgrade.isChecked():
                for pkg in packages:
                    if self._package_exists_in_path(pkg, maya_system_site): skipped.append((pkg, maya_system_site, "[Maya Install]"))
                    elif self._package_exists_in_path(pkg, maya_site): skipped.append((pkg, maya_site, "[Maya Site]"))
                    elif self._package_exists_in_path(pkg, target): skipped.append((pkg, target, "[Custom Target]"))
                    elif self._package_exists_in_path(pkg, user_site): skipped.append((pkg, user_site, "[User Site]"))
                    else: to_install.append(pkg)
            else: to_install = packages

            for pkg, path, label in skipped:
                self.log_signal.emit("⏭ Skipped '{}' — already found in {} ({})\n   Use --upgrade to force reinstall.".format(pkg, label, path))
            ...
            if requires_admin and os.name == "nt":
                self.log_signal.emit("🔐 Requesting Administrator permission for Maya Install. Approve the Windows UAC prompt to continue.")
                result = self._run_elevated_process(cmd)
            else: result = subprocess.run(cmd, capture_output=True, text=True)

            if result.stdout: self.log_signal.emit(result.stdout)
            if result.stderr:
                real_errors = [l for l in result.stderr.strip().splitlines() if not l.startswith("WARNING: You are using pip version") and not l.startswith("You should consider upgrading")]
                if real_errors: self.log_signal.emit("[STDERR]\n" + "\n".join(real_errors))

            if result.returncode == 0:
                self.log_signal.emit("\n✅ Install completed successfully!")
                if requires_admin: self.log_signal.emit("🔐 Installed into the Maya system site-packages folder.")
                if install_path:
                    if install_path not in sys.path:
                        sys.path.insert(0, install_path)
                        self.log_signal.emit("📂 Added to sys.path: " + install_path)
                    else: self.log_signal.emit("📂 Already in sys.path: " + install_path)
            ...
            for pkg in packages:
                if self._package_exists_in_path(pkg, maya_system_site):
                    self.log_signal.emit("🚫 Cannot uninstall '{}' — it is installed in the Maya system folder:\n   {}\n   This path requires Administrator rights to modify.\n   Run Maya (or your file manager) as Administrator if you really need to remove it.".format(pkg, maya_system_site))
                    continue
                removed_any = False
                for folder in [maya_site, target, user_site]:
                    if self._package_exists_in_path(pkg, folder):
                        ...
                        removed_any = True
                if not removed_any: self.log_signal.emit("⚠ '{}' not found in any writable path.".format(pkg))
    ...
    def _start_install(self):
        self._set_busy(True)
        threading.Thread(target=self._run_pip, args=("install",), daemon=True).start()

    def _start_uninstall(self):
        self._set_busy(True)
        threading.Thread(target=self._run_pip, args=("uninstall",), daemon=True).start()
